...
public class GeolocationControllerIntegrationTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task GivenValidAndInvalidIpAddresses_WhenGetGeolocations_ThenGeolocationsReturned(string[] ipAddresses, string?[] expected)
    {
        // Given
        var client = _factory.CreateClient();
        var request = new GetGeolocationsRequest(ipAddresses);
    ...
    [Fact]
    public async Task GivenInvalidIpAddresses_WhenGetGeolocations_ThenGeolocationsReturned()
    {
        // Given
